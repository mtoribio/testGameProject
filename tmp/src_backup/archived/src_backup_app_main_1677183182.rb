...
class Game
  ...
  def defaults
    ...
    ## Animation
    player.run_start_at        ||= state.tick_count
    player.animation_start_at  ||= state.tick_count
    state.animation_action     ||= :idle
  ...
  def render_player
    render_animation('sprites/player/idle.png', 10) if state.animation_action == :idle
    render_animation('sprites/player/run.png', 8)  if state.animation_action == :run
    render_animation('sprites/player/jump.png', 8) if state.animation_action == :jump
    render_animation('sprites/player/jump.png', 8) if state.animation_action == :fall
  end

  def render_animation(path, how_many_frames_in_sprite_sheet)
