...
class Game
  ...
  def render_player
    render_circle_animation('sprites/player/idle.png', 10) if state.animation_action == :idle
    render_circle_animation('sprites/player/run.png', 8)  if state.animation_action == :run
    render_jump_animation('sprites/player/jump.png', 8) if state.animation_action == :jump
    render_animation('sprites/player/jump.png', 8) if state.animation_action == :fall
  end

  def render_jump_animation
    outputs.sprites << {
      x: player.x,
      y: player.y,
      w: player.size,
