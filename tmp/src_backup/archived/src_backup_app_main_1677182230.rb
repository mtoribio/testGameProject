...
class Game
  ...
  def defaults
    ...
    player.idle_start_at       ||= state.tick_count
  ...
  def render_player
    idle_animation if player.action == :standing
    run_animation  if player.action == :moving
    jump_animation if player.action == :jumping
    fall_animation if player.action == :falling
  end
  ...
  def idle_animation
    ...
    tile_index = player.idle_start_at.frame_index(
  ...
  def run_animation
    ...
    tile_index = player.run_start_at.frame_index(
      how_many_frames_in_sprite_sheet,
      how_many_ticks_to_hold_each_frame,
      should_the_index_repeat
    )
